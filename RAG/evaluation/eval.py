...
from rag_pipeline_implementation.answer import answer_question, fetch_context
    ...
    judge_messages = [
        {
            ...
            "content": (
                ...
                '{"feedback": "<string>", "accuracy": <float 1-5>, "completeness": <float 1-5>, "relevance": <float 1-5>}. '
...
If the answer is wrong, accuracy must be 1. Respond ONLY with a JSON object like:
{{"feedback": "...", "accuracy": 3.0, "completeness": 2.0, "relevance": 4.0}}""",
        },
    ]

    # format="json" is Ollama's native JSON mode; response_format=PydanticModel is not supported for local Ollama models
    # AnswerEval.model_validate_json() below still parses the response into a structured AnswerEval object
    judge_response = completion(model=MODEL, messages=judge_messages, format="json")
